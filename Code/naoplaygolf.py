...
def TriangleCalculation2(x, s, alpha):
    ...
    if (alpha > 0.0):
        ...
        if theta >= math.pi / 2:
            ...
            # turnAngle1 = -math.pi + theta2
            print(turnAngle1)
            dis1 = x * math.sin(theta2) + 0.16  # 修正值
            # turnAngle2 = -math.pi-turnAngle1
            turnAngle2 = 0
    ...
    if (alpha < 0.0):
        ...
        motionPrx.moveTo(0.0, 0.0, -math.pi / 2,
                         [["MaxStepX", maxstepx],
                          ...
                          ["TorsoWy", torsowy]])
        motionPrx.setMoveArmsEnabled(False, False)
        motionPrx.moveTo(0.6, 0.0, 0.0,  # 此处参数可调，设置搜寻距离
                         [["MaxStepX", maxstepx],
                          ["MaxStepY", maxstepy],
                          ["MaxStepTheta", maxsteptheta],
                          ["MaxStepFrequency", maxstepfrequency],
    ...
    if (alpha > 0.0):
        ...
        motionPrx.moveTo(0.0, 0.0, math.pi / 2,
                         [["MaxStepX", maxstepx],
                          ...
                          ["TorsoWy", torsowy]])
        motionPrx.setMoveArmsEnabled(False, False)
        motionPrx.moveTo(0.6, 0.0, 0.0,  # 此处参数可调
                         [["MaxStepX", maxstepx],
                          ["MaxStepY", maxstepy],
                          ["MaxStepTheta", maxsteptheta],
                          ["MaxStepFrequency", maxstepfrequency],
...
while True:
    ...
    if (allmarkdata[4] == 0):
        ...
        dx = (h - 0.1) / (math.tan(thetav))  # dx作为了三角形的一条边
        if dx > 0.1:
            motionPrx.moveTo(dx-0.1, 0.0, 0.0, _stepStatue)
        motionPrx.angleInterpolationWithSpeed("HeadPitch", 0.0, 0.3)
        motionPrx.angleInterpolationWithSpeed("HeadYaw", 0.0, 0.3)
